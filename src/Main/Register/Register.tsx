    ...
    <div className={styles.Register} id="sign-up">
      ...
      <form onSubmit={handleSubmit(Submit)}>
              ...
              positions.map((key) => (
                <li key={key.id} className={styles.radioBtn}>
                  <input
                    ...
                    name={key.name}
                    onChange={(e) => {}}
                    checked={radio === key.id}
                  />
                  <label onClick={() => setRadio(key.id)} htmlFor={key.name}>
                    {key.name}
                  </label>
                </li>
              ))}
        ...
        <label htmlFor="upload" className={styles.Register__Upload}>
          ...
          <div className={styles.Upload_second}>
            {fileSelect.name || "Upload your photo"}
          </div>
        </label>
        <input
          className={styles.Register__UploadInput}
          id="upload"
          type="file"
          accept=".jpg, .jpeg"
          onChange={(e: any) =>
            e.target.files[0] && setFileSelect(e.target.files[0])
          }
        ></input>
